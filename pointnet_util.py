...
def minkowski_distance(src, dst, p=2):
    """
    Calculate minkowski distance.  This generalizes most distance
     metrics.  p = 1 we have manhatten distance, p=2 we have euclidean
...
def cosine_sim(src, dst):
    """
    Calculate the cosine similarity between src and
    dst
    ...
    Output:
        dist: per-point square distance, [B, N, M]
    """
    return torch.matmul(src, dst.transpose(1,2)) / ((torch.linalg.vector_norm(src[:, :, None])) * (torch.linalg.vector_norm(dst[:, :, None])))
